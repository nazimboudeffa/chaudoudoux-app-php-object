 ...
 * @return void;
 */
function chaudoudoux_register_language($code = '', $file) {
		if(isset($code) && isset($file)) {
				global $Chaudoudoux;
				$Chaudoudoux->locale[$code][] = $file;
		}
}
 ...
 * @return void;
 */
function chaudoudoux_register_languages($code, $params = array()) {
		global $Chaudoudoux;
		if(isset($Chaudoudoux->localestr[$code], $code)) {
				$params = array_merge($Chaudoudoux->localestr[$code], $params);
		}
		$Chaudoudoux->localestr[$code] = $params;
}
 ...
 * @return array
 */
function chaudoudoux_locales() {
		global $Chaudoudoux;
		if(!isset($Chaudoudoux->locale)) {
				return false;
		}
		foreach($Chaudoudoux->locale as $key => $val) {
				$keys[] = $key;
		}
		if(!empty($keys)) {
				return $keys;
 ...
 * @return string
 */
function chaudoudoux_print($id = '', $args = array()) {
		global $Chaudoudoux;
		$id   = strtolower($id);
		$code = chaudoudoux_site_settings('language');
		if(!empty($Chaudoudoux->localestr[$code][$id])) {
				$string = $Chaudoudoux->localestr[$code][$id];
				if($args) {
 ...
 * @return void
 */
function chaudoudoux_default_load_locales() {
		global $Chaudoudoux;
		$active = chaudoudoux_site_settings('language');
		if(chaudoudoux_site_settings('cache') == 1) {
				$system_locale_cache = chaudoudoux_get_userdata("system/locales/");
				...
				if(file_exists($cached_locale)) {
						...
						if(!json_last_error()) {
								$Chaudoudoux->localestr[$active] = $cached_locale_array;
						} else {
							throw new exception('Can not decode the cached language file');	
						}
				}
		} else {
				if(isset($Chaudoudoux->locale[$active])) {
						foreach($Chaudoudoux->locale[$active] as $locales) {
								if(is_file($locales)) {
 ...
 * @return array
 */
function chaudoudoux_standard_language_codes() {
		return array(
				'aa',
				'ab',
				'af',
 ...
 * @return array
 */
function chaudoudoux_get_available_languages() {
		...
		//load core framework languages
		foreach($codes as $code) {
				$file = $path->locale . "chaudoudoux.{$code}.php";
				if(is_file($file)) {
						$core_langs[] = $code;
				}
		}
